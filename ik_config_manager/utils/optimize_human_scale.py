...
def optimize_human_scale_table(human_data, robot_centers, body_names, ik_config, human_root_name="pelvis", 
    ...
    # 创建基于人体关节的对称分组, 比如Left_Knee和Right_Knee会分到Knee这个组中
    ...
    group_scale_params = Variable(initial_scales_tensor, requires_grad=True)    # 告诉计算图这个参数的梯度是要被更新的，是需要被训练的参数
...
def create_human_based_symmetric_groups(human_scale_keys):
    ...
    if center_joints:
        scale_groups['center_group'] = center_joints    # 把骨盆、脊柱、脖子、头等位于身体中轴线上的关节放到center_group中
    
    # 提取对称部位名称
    # 移除前缀，比如 Left_Knee -> Knee
    ...
    # 找到对称部位
    symmetric_parts = set(left_parts.keys()) & set(right_parts.keys())  # 寻找配对，通过交集的方式
    ...
    for table_key in ["ik_match_table1", "ik_match_table2"]:
        ...
        for robot_link, config in ik_config[table_key].items():
            ...
            if (robot_link in robot_link_indices and human_bone_name in scale_keys):
                ...
                # 存储机器人目标位置
                robot_positions[(robot_link, human_bone_name)] = robot_pos  # 存储机器人与人体连杆对应时，机器人连杆的全局位置
