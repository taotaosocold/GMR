"""
    python scripts/adjust_robot_height_by_gravity.py --robot_motion_path input_pkl --robot casbot_skeleton --rate_limit --save_path output
"""
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        choices=["unitree_g1","unitree_g1_with_hands", "unitree_g1_with_wrist_roll", "unitree_g1_sim2sim",  "unitree_g1_23", 
                ...
                "mini_pis", "casbot_skeleton", "marathon_001"
            ],
        default="unitree_g1",
    )
    ...
    with open(args.robot_motion_path, 'rb') as f:
        data = pickle.load(f)
    ...
    qpos_mtx = np.concatenate([root_pos, root_rot, dof_pos], axis=1)
    ...
    if args.plot:
        ...
        plt.plot([i * fps for i in range(qpos_mtx.shape[0])], qpos_mtx[:,2], label='before', alpha=0.7)
    
    qpos_mtx, minidxes, maxidxes = adjust_root_z(qpos_mtx)
    qpos_mtx = piecewise_savgol(qpos_mtx, window=6, poly=3)

    if args.plot:
        plt.plot([i * fps for i in range(qpos_mtx.shape[0])], qpos_mtx[:,2], label='after', alpha=0.7)
        plt.scatter([i * fps for i in minidxes], qpos_mtx[:,2][minidxes], label='local min', color='red', marker='*')
        plt.scatter([i * fps for i in maxidxes], qpos_mtx[:,2][maxidxes], label='local max', color='green', marker='*')
        plt.xlabel('Time (s)')
    ...
    if args.save_path is not None:
        ...
        # 将调整后的 qpos_mtx 拆分回原始格式
        root_pos = qpos_mtx[:, :3]            # 前3列为根位置
        root_rot = qpos_mtx[:, 3:7]           # 紧接着4列为根旋转（xyzw）
        root_rot = root_rot[:, [1, 2, 3, 0]]
        dof_pos  = qpos_mtx[:, 7:]            # 其余为关节位置
        # 重建与原pickle相同的字典
        data_out = {
            'root_pos': root_pos,
            'root_rot': root_rot,
